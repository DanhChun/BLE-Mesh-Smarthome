        ...
        case ESP_BLE_MESH_MODEL_OPERATION_EVT:
            switch (param->model_operation.opcode) {
                ...
                case ESP_BLE_MESH_CUSTOM_SENSOR_MODEL_OP_SET:
                    ...
                    //hàm xử lý bản tin nhận về từ client
...
static void parse_received_data(esp_ble_mesh_model_cb_param_t *recv_param, model_sensor_data_t *parsed_data) {
    ...
    // ESP_LOGW("PARSED_DATA", "Device Num   = %d", parsed_data->device_num);
    // ESP_LOGW("PARSED_DATA", "Temperature  = %f", parsed_data->temperature);
    // ESP_LOGW("PARSED_DATA", "Humidity     = %f", parsed_data->humidity);
    // ESP_LOGW("PARSED_DATA", "Relay 1      = %d", parsed_data->relay_1);
    // ESP_LOGW("PARSED_DATA", "Brightness   = %f", parsed_data->brightness);
    // ESP_LOGW("PARSED_DATA", "Relay 2      = %d", parsed_data->relay_2);
    // ESP_LOGW("PARSED_DATA", "RFID         = %d", parsed_data->rfid);
    // ESP_LOGW("PARSED_DATA", "Door         = %d", parsed_data->door);

    // gửi bản tin vào queue cho 1 task khác xử lý
...
esp_err_t ble_mesh_custom_sensor_server_model_message_status(model_sensor_data_t stt_data) {
    ...
    opcode = ESP_BLE_MESH_CUSTOM_SENSOR_MODEL_OP_STATUS;
    ...
    //         sizeof(stt_data), (uint8_t *)&stt_data, 0, false, ROLE_NODE);

    err = esp_ble_mesh_server_model_send_msg(custom_models, &ctx, opcode,
            sizeof(stt_data), (uint8_t *)&stt_data);
