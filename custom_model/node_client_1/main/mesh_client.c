...
static void ble_mesh_custom_sensor_client_model_cb(esp_ble_mesh_model_cb_event_t event,
    ...
    switch (event) {
        ...
        //xử lý bản tin nếu server publish bản tin
...
//hàm xử lý bản tin nhận về từ server
static void parse_received_data(esp_ble_mesh_model_cb_param_t *recv_param, model_sensor_data_t *parsed_data) {
    ...
    ESP_LOGI(TAG, "Device Number: %d", parsed_data->device_num);
    // ESP_LOGI(TAG, "Temperature: %f", parsed_data->temperature);
    // ESP_LOGI(TAG, "Humidity: %f", parsed_data->humidity);
    ESP_LOGI(TAG, "Relay 1: %d", parsed_data->relay_1);
    // ESP_LOGI(TAG, "Brightness: %f", parsed_data->brightness);
    // ESP_LOGI(TAG, "Relay 2: %d", parsed_data->relay_2);
    // ESP_LOGI(TAG, "RFID: %d", parsed_data->rfid);
    // ESP_LOGI(TAG, "Door: %d", parsed_data->door);

    // if (parsed_data->device_num == 1) 
    // {
        ESP_LOGI(TAG, "Device 1 received data");
        Relay_Control(parsed_data->relay_1);
    // } 
}
